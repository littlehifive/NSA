...
```{r}
#load("/Users/michaelfive/Library/CloudStorage/GoogleDrive-wuzezhen33@gmail.com/My Drive/Nepal SA Study/Analysis/model/gr6-10_survey_gpa_0730.RData")
```
...
df <- df |> 
  ...
  mutate(class_id = case_when(
           class_id %in% c(1:5) ~ paste0("Baglung Grade ", class_id + 5),
           class_id %in% c(8:12) ~ paste0("Kathmandu Grade ", class_id - 2),
           class_id %in% c(15:19) ~ paste0("Pokhara Grade ", class_id - 9)
         )) |> 
  mutate(class_id = factor(class_id,
                           levels = c(
                             paste0("Baglung Grade ", 6:10),
                             paste0("Kathmandu Grade ", 6:10),
                             paste0("Pokhara Grade ", 6:10)
                           ))) |> 
